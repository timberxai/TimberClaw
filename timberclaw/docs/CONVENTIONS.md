...
- **TimberClaw 工作主分支**：`cursor`（日常开发、PR 合并目标；禁止直接推送）
- **`main`**：保留用于上游 OpenHands 同步、发布节奏或仓库策略对齐；TimberClaw 功能改动**不直推 `main`**，通过 PR 合入 `cursor`
...
- 自 **2026-04-19** 起，所有 TimberClaw 相关实现与文档 PR **以 `cursor` 为 base / 合并目标**（含 Coding Agent 产出）。
- 开工前请执行：`git fetch origin && git checkout cursor && git pull origin cursor`，再从此点创建 `tc/<spec_version>/<short_desc>` 等工作分支。
- **`main`**：不在此分支上开展 TimberClaw 日常迭代；需要与上游或发布对齐时，由 Platform Engineer 按单独流程处理（可另开 `upstream/sync-*` 等分支）。
...
禁止：直接向 **`cursor`** 或 **`main`** 推送；跨工单合并一个分支；branch 名里出现中文。
...
1. Platform Engineer 从 **上游 OpenHands**（`github.com/All-Hands-AI/OpenHands`）的 `main` 拉取最新 commit；在本仓库开 `upstream/sync-<yyyymmdd>` 分支
2. 合并冲突必须逐点评估：
...
4. 通过后以 MR **合入本仓库 `cursor`**；若需与对外发布或 GitHub 默认策略对齐，再由 Platform Engineer 将 `cursor` 同步至 `main`（策略另定）。MR 必须 Owner 签字。
