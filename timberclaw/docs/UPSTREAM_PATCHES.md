...
| 2026-04-20 | `frontend/.eslintrc` | `src/timberclaw/**` 增加 `no-restricted-syntax`：`merge request` / `rc-` 字面量拦截（W-B-00） | `[upstream-patch]` |
| 2026-04-20 | `frontend/.eslintrc` | W-B-00：扩展 `no-restricted-syntax`（`commit` / `migration` / `diff` / `Preview` / `Prod` / `PR ` / `pull request` + JSXText） | `[upstream-patch]` |
...
| 2026-04-20 | `docker-compose.yml` | `TC_GITLAB_ENABLE_WRITE` 默认 `0`，显式开启后才允许 `smoke-write` 演练 | `[upstream-patch]` |
| 2026-04-20 | `.github/workflows/timberclaw-builder.yml` | TimberClaw：`docker compose` 内 pytest、`tc_wave_a_check.sh`、`frontend/src/timberclaw` ESLint；触发含 `main`/`cursor` + paths 过滤 | `[tc-agent]` |
