...
>
> **阅读顺序**：本文件只规定"怎么工作"。要理解"为什么这样工作"，必须先读 `docs/PRD.md`（**特别是 V1.6 新增 §15 Coding Agent 工作模式**）。`docs/EXECUTION_TODO.md` 是当下唯一的工单调度面（PRD §15.1）。`docs/CONVENTIONS.md` 定义仓库结构纪律。`docs/BACKLOG.md` 提供工单语义。
...
每次新接一个工单（或重新激活一个会话），**按顺序**做完以下 6 件事才能动手改代码：

1. **读 PRD**：至少读 `docs/PRD.md` **§15（Coding Agent 工作模式）**、§2（产品定义）、§4（角色）、§5（核心原则）、§11（最小化要求），再对应读工单涉及的功能章节
2. **从 EXECUTION_TODO Pickup**：在 `docs/EXECUTION_TODO.md` 按 PRD §15.3 顺序挑选下一张工单；不得绕过该文件凭记忆继续
3. **读工单语义**：到 `docs/BACKLOG.md` 找对应 `M?-??`，通读其"范围 / 不做 / 依赖 / 对齐 PRD / 验收标准"
4. **读约定**：读 `docs/CONVENTIONS.md` 第 2、3、4 节，弄清楚代码该放哪、分支怎么命名、commit 怎么写
5. **确认依赖**：工单的"依赖"字段里列出的前置工单必须已经合入 **`cursor`**；若否，**停止并报告**（PRD §15.6），不得自作主张先做依赖
6. **规划再动手**：写代码前先在 PR 描述里产出一份「实现计划 + 命中 Range 列表」，确认未越过工单 Range，再进入编码
...
1. **禁止**绕过 Git / PR 流程（直接改 **`cursor` / `main`**、用强推、用 rebase 覆写共享分支）
...
```
  ...
  └─────────────────────────────────────────────┘
                     ...
                     ▼
  ┌─────────────────────────────────────────────┐
  │  4. 从 cursor 拉最新后新开 tc/<spec>/<desc> 分支│
  └─────────────────────────────────────────────┘
                     │
                     ▼
  ┌─────────────────────────────────────────────┐
